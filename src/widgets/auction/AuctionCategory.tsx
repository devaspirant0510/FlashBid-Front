...
const AuctionCategory: FC<Props> = ({category, type}) => {
    ...
    if (type === "live") {
                    ...
                    {category.map((value, index) => {
                        return (
                            <div key={index} onClick={() => onClickItem(index)}>
                                <CategoryItem type={'live'} value={value.name} active={selected[index]}/>
                            </div>
                        )
                    })}
        ...
        return <>
            <div className={'flex flex-col w-full items-center rounded-b-4xl border-1 bg-udark'}>
                <div className={'text-[#ED6C37] text-2xl font-bold  mt-8'}>
                    블라인드 경매
                </div>
                <div className={'text-[#FFBB9F] text-sm'}>
                    긴장감이 넘치는 채팅 속에서 경매 참가들의 심리를 파악해 상품을 낙찰받아 가세요!
                </div>
                <div className="flex items-center gap-2 my-4 w-full">
                    <div className="flex-grow h-px bg-[#CC7451]"/>
                    <span className="text-sm text-[#CC7451] whitespace-nowrap">카테고리</span>
                ...
                <div className={'flex bg-[#D5D5D6] rounded-full gap-4 py-2 px-4'}>
                    {category.map((value, index) => {
                        return (
                            <div key={index} onClick={() => onClickItem(index)}>
                                <CategoryItem type={'blind'} value={value.name} active={selected[index]}/>
                            </div>
                        )
                    })}
                ...
                <div className={'flex justify-between items-center w-full px-4 mb-4'}>
                    ...
                    <div className={'border-1 flex justify-between p-2 rounded-full text-sm items-center border-[#9F9F9F] text-[#9F9F9F]'}>
                        필터
