...
const BiddingDialog: FC<Props> = ({client}) => {
    ...
    const onClickBid = useCallback(() => {
        const jsondata = {
            ...
            bid:{
                price:value,
                prevPrice:data?.data?.currentPrice?data.data.currentPrice:data?.data?.auction.startPrice
            }
        }
        client.publish({destination: "/app/chat/send/" + auctionId, body: JSON.stringify(jsondata)})
        setOpen(false)
        setValue("");
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>
                        <div className={'flex'}>
                            <img className={'object-cover w-24 h-24 rounded-lg'} width={50} height={50}
                        ...
                        <div className={'flex'}>
                            <span>현재 가격</span>
                            <div>{data.data.currentPrice?data.data.currentPrice.toLocaleString():data.data.auction.startPrice.toLocaleString()}p</div>
                        </div>
                    </DialogTitle>
                    <DialogDescription>
