...
const AuctionWishListButton: FC<Props> = ({isWishListed, auctionId}) => {
    ...
    const onClickWishList = useCallback(async () => {
        ...
        if (result.status === 200 ) {
            ...
            // 위시리스트 상태를 업데이트
            console.log(auctionId)
            queryClient.setQueryData(['api', 'v1', 'auction', Number(auctionId)], (prev: ApiResult<AuctionInfoData>) => {
    ...
    return (
        <>
            <button
                className="absolute right-4 bottom-4 bg-white rounded-full p-1 flex items-center justify-center shadow">
                {isWishListed ?
                    <HeartIcon color="#e53e3e" size={25} onClick={onClickCancelWishList}
