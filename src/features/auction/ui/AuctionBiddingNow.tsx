...
const AuctionBiddingNow: FC<Props> = ({data}) => {
    ...
    const onClickBuyNow = useCallback((data:AuctionInfoData,client:Stomp)=>{
        const jsondata = {
        ...
        client.publish({destination: "/app/chat/send/"+data.auction.id  , body: JSON.stringify(jsondata)})
                    ...
                    <section className={'flex flex-col bg-ubackground1 p-4'}>
                        ...
                        <div className={'flex justify-between mb-2'}>
                            <div className={'text-usecondary'}>현재가
                                :{data.lastBiddingLog? data.lastBiddingLog.price.toLocaleString() : data.auction.startPrice.toLocaleString()}p
                            </div>
                        ...
                        <div className={'rounded-full bg-ubackground2 flex justify-between items-center'}>
                            <div
                                className={'bg-usecondary rounded-full text-white py-2 px-6 font-bold whitespace-nowrap'}>
                                입찰 가격
                            </div>
                        ...
                        <Button className={'bg-uprimary py-6 text-xl mt-4'} onClick={(e)=>onClickBuyNow(data,client)}>
                            입찰하기
                        </Button>
                    </section>
                )
