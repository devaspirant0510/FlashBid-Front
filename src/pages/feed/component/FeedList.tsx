...
import { getServerURL, httpFetcher, axiosClient } from '@shared/lib';
...
import { useAuthStore } from '@shared/store/AuthStore.ts'; // AuthStore import
...
const FeedList = () => {
    ...
    const [likeStatusMap, setLikeStatusMap] = useState<{ [key: number]: { isLiked: boolean; count: number } }>({});
    ...
    return (
        <div className='px-4 py-6 flex flex-col items-center gap-6'>
            ...
            {data.data.map((v) => {
                ...
                const isAuthor = currentUserId !== null && v.feed.user.id !== undefined && currentUserId === v.feed.user.id;
